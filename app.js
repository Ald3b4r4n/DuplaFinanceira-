...
function calcularNovaProximaRecorrencia(data, tipo) {
...
async function adicionarTransacao() {
  ...
  const transacao = {
    data: new Date(dataInput).toLocaleDateString('pt-BR'),
      ...
      if (data.pago) {
        tabelaHTML += `
          <tr>
            ...
            <td>${data.categoria}</td>
            <td>${data.responsavel}</td>
            <td>${data.pago ? '✔️ Pago' : '❌ Pendente'}</td>
            <td>${data.recorrente ? (data.tipoRecorrencia === "semanal" ? "🔁 Semanal" : "🔁 Mensal") : '❌'}</td>
            <td><button class="delete-btn" onclick="window.removerTransacao('${docItem.id}')">🗑️</button></td>
          </tr>
        `;
...
async function exportarParaPDF() {
  try {
    ...
    doc.autoTable({
      head: [['Data', 'Descrição', 'Valor', 'Categoria', 'Responsável', 'Pago', 'Recorrência']],
      body: dados,
      startY: 45,
      styles: { fontSize: 8 },
...
async function exportarParaExcel() {
  try {
    ...
    let csv = 'Data,Descrição,Valor,Categoria,Responsável,Pago,Recorrência\n';
    let temDados = false;
    ...
    querySnapshot.forEach(docItem => {
      ...
      if (deveIncluir) {
        temDados = true;
        csv += `"${data.data}","${data.descricao}",${data.valor},"${data.categoria}","${data.responsavel}","${data.pago ? 'Sim' : 'Não'}","${data.recorrente ? (data.tipoRecorrencia === "semanal" ? "Semanal" : "Mensal") : 'Não'}"\n`;
      }
    });
